...
| Intrinsic (natural-equation) curves | 🟢 | `curves` | `Curve2::Intrinsic` + `CurvatureLaw` — curvature as a function of arc length. Constant, polynomial (clothoid/Bloss/cubic), sinusoid, composite polynomial-plus-harmonics, and piecewise laws over one domain. Exact and symbolic: differentiate, mirror, and total turning in closed form. Position is deliberately absent — it is a Fresnel-type integral that belongs to an evaluator that can state a tolerance. |
