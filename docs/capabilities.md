...
| Scalar geometry generation | Implemented broad discrete path plus focused exact construction | `axiolid-construct` builds exact extrusions of filled/hollow rectangles with sharp or rounded corners, positive-axis circles and ellipses, arbitrary polygon and arc contours with holes, the I/asymmetric-I/T/U/L/Z/C/trapezium sections, derived (transformed) profiles, composite profiles, and centre-line profiles, exact full-turn revolutions of contour-lowerable profiles, and the certified one-owner affine trace arrangement. Other profile/path families remain discrete or fail closed; see [ADR 0024](/adr/0024-exact-brep-result-contracts) and [ADR 0029](/adr/0029-certified-trace-topology-integration) |
...
| Exact B-rep operation results | Focused extrusion, revolution, boolean, chamfer and affine-arrangement slices; general operations planned | `axiolid-construct`, `axiolid-brep`; every exact extrusion family (see "Sweeps / extrusions") retains analytic planes/cylinders, closed trims, pcurves, and native spans through exact graph compilation. Full-turn revolution of any profile that lowers to a contour (contours, sections, derived, composite, centre-line) is exact: straight segments sweep planes, cylinders and cones, and arc segments sweep tori. Boolean over two COAXIAL prisms is exact via the planar overlay, including a wall with an interior opening; stepped results (unions with differing spans, tools shorter than the subject) and disconnected results refuse by name. Constant-distance chamfer on one straight vertical edge of an extruded sharp rectangle is exact. One bounded surface-pair slice also returns analytic supports and a residual certificate. Partial-turn revolutions, profiles crossing the revolution axis, revolution of circles, ellipses, hollow rectangles or sections with holes, non-coaxial or non-prism boolean operands, general sweeps, instances, authored B-rep graph nodes, and curved intersections remain unsupported ([#111](https://github.com/axiolid/kernel/issues/111)) |
...
| Sweeps / extrusions / revolutions / lofts | Broad discrete reference path plus focused exact extrusion, revolution and straight sweep | `axiolid-construct` exactly extrudes filled/hollow rectangles with sharp or rounded corners, positive-axis circles and ellipses, arbitrary polygon and arc contours with holes (`IfcArbitraryClosedProfileDef`, `IfcArbitraryProfileDefWithVoids`), the I/asymmetric-I/T/U/L/Z/C/trapezium sections, derived (transformed) profiles, composite profiles (members unioned, holes kept), and centre-line profiles. Full-turn revolution is exact for every profile that lowers to a contour. Fixed-reference sweeps along a STRAIGHT directrix delegate to the extrusion they are, so they inherit every extrusion family. Still refused by name: oblique circle/ellipse extrusion, disjoint or arc-bearing composite members, curved directrices, partial turns, and lofts ([#111](https://github.com/axiolid/kernel/issues/111)). The L2 result boundary keeps exact B-rep and tolerance-bearing tessellation requests distinct and never substitutes a mesh for exact output |
