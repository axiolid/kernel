...
compiler memoises supported graph extrusion and full-turn revolution roots as
`ExactBRep`, so planar walls stay planes and arc walls stay cylinders through
compilation, for every profile family listed under "Sweeps / extrusions" below. Its cache cannot contain `TriMesh`; unsupported
...
claims as applying only to the supported extrusion and revolution families, the
coaxial-prism boolean, and the independently certified affine trace arrangement
described below.
...
| Scalar geometry generation | Implemented broad discrete path plus focused exact construction | `axiolid-construct` builds exact extrusions of filled/hollow rectangles with sharp or rounded corners, positive-axis circles and ellipses, arbitrary polygon and arc contours with holes, the I/asymmetric-I/T/U/L/Z/C/trapezium sections, derived (transformed) profiles, composite profiles (members unioned exactly, arcs and member holes kept, disjoint members as separate solids), and centre-line profiles, exact full-turn revolutions of contour-lowerable profiles, circles and sections with holes (each hole a toroidal cavity), and the certified one-owner affine trace arrangement. Other profile/path families remain discrete or fail closed; see [ADR 0024](/adr/0024-exact-brep-result-contracts) and [ADR 0029](/adr/0029-certified-trace-topology-integration) |
| Scalar graph compilation | Implemented separate mesh and focused exact paths | `axiolid-mesh-compile`; `ReferenceExactCompiler` owns a per-batch `NodeId -> ExactBRep` cache and supports the exact extrusion and full-turn revolution families above; exact booleans accept only sharp-rectangle prisms. `ReferenceMeshCompiler` remains the explicit tolerance-driven discrete path. Unsupported exact roots refuse instead of tessellating; see [ADR 0020](/adr/0020-exact-brep-kernel-model) |
...
| Mesh Boolean execution | Optional provider | `axiolid-mesh-boolean-boolmesh`; limited to its mesh contract and tests. The algorithm is absorbed into the crate (ADR 0047), ported from `boolmesh` 0.1.9 under MPL-2.0 with upstream attribution preserved, so its hot paths and defects are ours to change |
...
| Exact B-rep operation results | Focused extrusion, revolution, boolean, chamfer and affine-arrangement slices; general operations planned | `axiolid-construct`, `axiolid-brep`; every exact extrusion family (see "Sweeps / extrusions") retains analytic planes/cylinders, closed trims, pcurves, and native spans through exact graph compilation. Full-turn revolution of any profile that lowers to a contour (contours, sections, circles, hollow rectangles and circles, derived, composite, centre-line) is exact: straight segments sweep planes, cylinders and cones, arc segments sweep tori, and each hole sweeps a closed cavity carried as a void shell. Boolean over two COAXIAL prisms is exact via the planar overlay, including a wall with an interior opening; stepped results (unions with differing spans, tools shorter than the subject) and disconnected results refuse by name. Constant-distance chamfer on one straight vertical edge of an extruded sharp rectangle is exact. One bounded surface-pair slice also returns analytic supports and a residual certificate. Partial-turn revolutions, profiles crossing the revolution axis, revolution of ellipses, non-coaxial or non-prism boolean operands, general sweeps, instances, authored B-rep graph nodes, and curved intersections remain unsupported ([#111](https://github.com/axiolid/kernel/issues/111)) |
...
| Sweeps / extrusions / revolutions / lofts | Broad discrete reference path plus focused exact extrusion, revolution and straight sweep | `axiolid-construct` exactly extrudes filled/hollow rectangles with sharp or rounded corners, positive-axis circles and ellipses, arbitrary polygon and arc contours with holes (`IfcArbitraryClosedProfileDef`, `IfcArbitraryProfileDefWithVoids`), the I/asymmetric-I/T/U/L/Z/C/trapezium sections, derived (transformed) profiles, composite profiles (members unioned exactly over one arc arrangement: arc members stay arcs, member openings stay open, disjoint members become separate solids), and centre-line profiles. Full-turn revolution is exact for every profile that lowers to a contour, including circles and sections with holes. Fixed-reference sweeps along a STRAIGHT directrix delegate to the extrusion they are, so they inherit every extrusion family. Still refused by name: oblique circle/ellipse extrusion, ellipse revolution, curved directrices, partial turns, and lofts. The L2 result boundary keeps exact B-rep and tolerance-bearing tessellation requests distinct and never substitutes a mesh for exact output |
| Certified B-rep distance | Implemented: `boundary_distance`, `boundary_clearance` | `axiolid-measure` (feature `exact`): an interval certain to contain the distance between the boundaries of two exact B-reps, with witness points on both, from branch and bound with exact projection bounds and normal-cone pruning ([ADR 0074](/adr/0074-certified-boundary-distance)). A clearance check against a limit answers Below, Above or Indeterminate, never rounding a value near the limit into a verdict. Isolated nearest pairs close to 1e-9; line contact (parallel columns) closes to about 1e-6 within the step budget. Distance is between boundaries, not a containment test |
| Mass properties | Implemented for meshes and exact B-reps, planar and curved | `axiolid-measure`: `MeshMeasure` implements `Measure<TriMesh>` (area, signed volume, volume centroid, second moments), and `exact_properties` measures an `ExactBRep` WITHOUT tessellating it: straight-edged planar faces by an exact fan, and cylinders, cones, spheres, tori, elliptical cylinders, B-spline faces and arc-bounded planar faces by Green's theorem over their own parameters with quadrature held to 1e-13 relative ([ADR 0073](/adr/0073-curved-face-mass-properties)). Verified against closed forms (Pappus, parallel-axis moments, hemispheres, cones, a half torus) and an exact-vs-mesh differential. A face whose pcurves bound no domain is refused by name; open shells are refused rather than assigned a plausible volume. Exact support is behind the `exact` feature so a mesh-only consumer does not acquire B-rep geometry |
| Mesh defect diagnosis | Implemented: located defects, not counts | `axiolid-inspect`; coplanar overlap decided exactly via `orient2d`; repair is separate |
...
| Connected-component decomposition | Implemented: `decompose` / `compose` | `axiolid-mesh`; connectivity is by shared vertex index, not geometric proximity; coincident-but-distinct vertices stay separate bodies until welded |
| Clearance / min gap | Implemented for triangle meshes | `axiolid-reference`; BVH-accelerated; reports clash as 0.0 and out-of-range as none |
| Point containment / winding number | Implemented, exact | `axiolid-inspect`; certified `orient3d` parity; signed, so inside-out shells are visible |
| Ray casting | Implemented for triangle meshes | `axiolid-inspect`; exact membership; only the hit parameter is f64 |
| Genus | Implemented for closed two-manifolds | `axiolid-inspect`; refuses meshes with boundary or non-manifold edges |
...
| Spatial, healing | Focused crates / staged capability | `axiolid-spatial`, `axiolid-heal`; consult each crate’s `PLAN.md`; do not infer broad CAD coverage |
...
| Parallel / SIMD | Opt-in context features | `axiolid-backend-cpu`; they require measurement and differential validation before performance claims |
...
- General exact B-rep results for unsupported graph/operation families (including
  general booleans, partial-turn revolutions, lofts, instances, authored B-rep
  roots, and sweeps along curved directrices), plus exact-result propagation
  beyond the focused extrusion, revolution and affine-arrangement slices.
