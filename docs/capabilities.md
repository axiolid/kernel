...
| Scalar geometry generation | Implemented broad discrete path plus focused exact construction | `axiolid-construct` builds exact extrusions of sharp filled/hollow rectangles, positive-axis circles and ellipses, arbitrary polygon and arc contours with holes, the I/asymmetric-I/T/U/L/Z/C/trapezium sections, derived (transformed) profiles, composite profiles, and centre-line profiles, exact full-turn revolutions of contour-lowerable profiles, and the certified one-owner affine trace arrangement. Other profile/path families remain discrete or fail closed; see [ADR 0024](/adr/0024-exact-brep-result-contracts) and [ADR 0029](/adr/0029-certified-trace-topology-integration) |
...
| Sweeps / extrusions / revolutions / lofts | Broad discrete reference path plus focused exact extrusion, revolution and straight sweep | `axiolid-construct` exactly extrudes sharp filled/hollow rectangles, positive-axis circles and ellipses, arbitrary polygon and arc contours with holes (`IfcArbitraryClosedProfileDef`, `IfcArbitraryProfileDefWithVoids`), the I/asymmetric-I/T/U/L/Z/C/trapezium sections, derived (transformed) profiles, composite profiles (members unioned, holes kept), and centre-line profiles. Full-turn revolution is exact for every profile that lowers to a contour. Fixed-reference sweeps along a STRAIGHT directrix delegate to the extrusion they are, so they inherit every extrusion family. Still refused by name: rounded rectangles, oblique circle/ellipse extrusion, disjoint or arc-bearing composite members, curved directrices, partial turns, and lofts ([#111](https://github.com/axiolid/kernel/issues/111)). The L2 result boundary keeps exact B-rep and tolerance-bearing tessellation requests distinct and never substitutes a mesh for exact output |
