...
## Coaxial surfaces of revolution (one identity, ten pairs)

Plane, cylinder, cone, sphere and torus are all surfaces of revolution.
Each is generated by rotating a **line or circle** in the `(rho, z)`
half-plane about its axis:

| surface | meridian profile |
|---|---|
| plane (normal along axis) | line `z = 0` |
| cylinder | line `rho = r` |
| cone | line `rho - z tan(alpha) = R0` |
| sphere | circle centred `(0, 0)` |
| torus | circle centred `(R_major, 0)` |

When two such surfaces are **coaxial**, the shared rotational symmetry
means they meet exactly where their profiles meet, and every profile
solution with `rho > 0` sweeps into a full circle perpendicular to the
axis. So one derivation covers every coaxial combination of the five
surfaces, rather than ten hand-written pair cases.

Profile intersection has only three shapes, all closed form:
line/line (Cramer), line/circle (perpendicular foot plus half-chord),
circle/circle (the radical-line identity, one dimension below
sphere/sphere).

Verified numerically against both operands' defining equations before
any code was written: coaxial sphere/cylinder `1.8e-15`, torus/plane
`2.7e-15`, cone/sphere `1.8e-15`, sphere/torus `7.1e-15`.

### What refuses, and why the reasons differ

- **Non-coaxial pairs** -> `UnsupportedPair`. A lateral axis offset
  destroys the shared symmetry and the intersection is generally a
  space quartic. Measured directly for unequal-radius crossing
  cylinders: the third singular value of the sampled curve is `2.83`,
  so no plane contains it.
- **Parabolic and hyperbolic cone sections** -> `UnrepresentableConic`.
  `Curve3` is `Line | Circle | Ellipse | Polyline | BSpline`; there is
  no parabola or hyperbola variant. The curve is exactly derivable but
  not expressible, which is a different failure from not being
  implemented, so it gets its own name. Substituting a near-ellipse or
  a fitted spline would be the dishonest option.
- **Oblique elliptical cone sections** -> `UnsupportedPair`. Genuinely
  derivable in closed form, but it needs an apex-offset construction
  that is not written yet. Named as a gap in this module, not in the
  geometry.

### Degeneracies

A profile root on or behind the axis (`rho <= 0`) generates no circle
and is dropped; if that leaves nothing, the result is a refusal rather
