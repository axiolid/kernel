...
MUTANTS = [
    ...
    ('poly: cmp_dyadic interior side flipped', 'poly.rs',
     ...
     '            Sign::Negative'),
    ('conic: resultant cross term sign flipped', 'conic.rs',
     '        let res = psub(\n            &pmul(&n, &n),',
     '        let res = padd(\n            &pmul(&n, &n),'),
    ('conic: tangency never detected', 'conic.rs',
     'doubled.degree().unwrap_or(0) >= 1 && u.sign_of(&doubled) == Sign::Zero',
     'false'),
    ('conic: shared real root with den accepted', 'conic.rs',
     '        if shared.degree().unwrap_or(0) >= 1 && !shared.real_roots().is_empty() {\n            continue;',
     '        if false {\n            continue;'),
    ('conic: side_of_line ignores den sign', 'conic.rs',
     '        sign_product(self.u.sign_of(&value), self.u.sign_of(&self.den))',
     '        self.u.sign_of(&value)'),
    ('conic: ellipse cross term sign flipped', 'conic.rs',
     'let qb = two.mul(&ux).mul(&uy).mul(&b2.sub(&a2));',
     'let qb = two.mul(&ux).mul(&uy).mul(&a2.sub(&b2));'),
    ('conic: line hits not ordered for negative alpha', 'conic.rs',
     '            let (first, second) = if exact(&al) == Sign::Positive {',
     '            let (first, second) = if true {'),
]
