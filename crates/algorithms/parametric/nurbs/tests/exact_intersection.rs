...
fn sphere_plane_gives_a_circle_on_both_surfaces() {
    ...
    assert_on_both(&sphere, &plane, &result.curve);
}
...
fn a_perpendicular_plane_cuts_a_cylinder_in_a_circle_of_its_own_radius() {
    ...
    assert_on_both(&cylinder, &plane, &result.curve);
}
...
fn two_planes_meet_in_a_line_on_both_surfaces() {
    ...
    assert_on_both(&first, &second, &result.curve);
}
...
fn an_unsupported_pair_is_refused_explicitly_rather_than_approximated() {
    ...
    let second = Surface::Sphere(Sphere {
        frame: frame(Point3::new(0.5, 0.0, 0.0), Vec3::new(0.0, 0.0, 1.0)),
        radius: 1.0,
    });

    // Sphere/sphere IS a circle, but this module does not derive it yet.
    // The refusal names that gap instead of silently fitting a curve.
    assert_eq!(
        exact_surface_intersection(&first, &second),
