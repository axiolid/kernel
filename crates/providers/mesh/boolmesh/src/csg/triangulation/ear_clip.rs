...
use crate::csg::{det2x2, is_ccw_2d, safe_normalize, Real, Vec2, Vec3u, K_BEST, K_PRECISION};
...
impl EarClip {
    ...
    fn process_ear(&mut self, v: &mut EvPtr, collider: &IdxCollider) {
        ...
        if v.borrow().is_convex(2. * self.eps) {
            // Edition 2021 holds the RHS temporary across the assignment, so
            // the `borrow()` is still live when `borrow_mut()` runs and the
            // RefCell panics. Edition 2024 drops it first. Evaluating into a
            // local reproduces the 2024 order explicitly.
            let cost = v.borrow().ear_cost(self.eps, collider);
            v.borrow_mut().cost = cost;
            let ptr = EvPtrMinCost(Rc::clone(v));
            v.borrow_mut().ear = Some(Rc::downgrade(&ptr.0));
            self.queue.insert(ptr);
            return;
